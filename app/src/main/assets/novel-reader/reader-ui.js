...
(function () {
    ...
    document.addEventListener("contextmenu", function (event) {
        ...
        if (modal.classList.contains("show")) hideImage();
        else if (image !== modalImage) showImage(image);
    });
    ...
    function readableElements() {
        var elements = Array.from(
            document.querySelectorAll(readableSelector),
    ...
    var excluded =
        'script,style,code,pre,textarea,button,[contenteditable="true"],' +
    ...
    function fixationLength(word) {
        ...
        return Math.max(
            thresholdIndex === -1
                ? word.length - fixationThresholds.length
                : word.length - thresholdIndex,
            0,
        );
    }
    ...
    function removeBionic() {
        document
            .querySelectorAll("b[data-tsundoku-bionic]")
            ...
            mutations.forEach(function (mutation) {
                mutation.addedNodes.forEach(function (node) {
                    if (node.nodeType === Node.TEXT_NODE) {
                        ...
                        !node.matches("b[data-tsundoku-bionic]")
                    ) {
                        applyBionic(node);
                    }
                });
    ...
    function setupImageLoading(img) {
        if (img.dataset.loadingHandlerAttached === "true") {
            return;
        }
        img.dataset.loadingHandlerAttached = "true";
        const markLoaded = () => {
    ...
    document.querySelectorAll("img").forEach(setupImageLoading);
    ...
    const observer = new MutationObserver((mutations) => {
        for (const mutation of mutations) {
            for (const node of mutation.addedNodes) {
                ...
                node.querySelectorAll("img").forEach(setupImageLoading);
            }
        }
    });
