...
class ContentPipelineTest {
    ...
    @Test
    fun `plain-text auto-split preserves existing whitespace`() {
        val text = "First  line.\n\nSecond\tline."
        assertEquals(text, TextSplitter.splitText(text, wordCount = 20, isHtml = false))
    }

    @Test
    fun `html auto-split leaves embedded script and style bodies untouched`() {
        val rawText = "word ".repeat(25) + "end."
        val html = "<script>$rawText</script><style>$rawText</style>"
        assertEquals(html, TextSplitter.splitText(html, wordCount = 20, isHtml = true))
    }
