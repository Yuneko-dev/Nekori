...
test('vertical fragments resolve duplicate ids inside the clicked chapter', async () => {
    ...
    try {
        ...
        const position = await page.evaluate(() => ({
            targetTop: document.querySelector('[data-chapter-id="2"] [id="note"]').getBoundingClientRect().top,
...
test('fragment round trips keep the paged viewport aligned', async (t) => {
    const browser = await chromium.launch({ executablePath: process.env.CHROMIUM_PATH });
    try {
        for (const direction of ['ltr', 'rtl']) for (const spread of ['single', 'double']) {
            await t.test(`${direction} ${spread}`, async () => {
                ...
                await page.evaluate(({ direction, spread }) => window.Tsundoku.runtime.readerLayout.configure({
                    enabled: true, spread, direction, chapterId: '1',
                }), { direction, spread });
                ...
                for (const href of ['#note', '#back', '#note', '#', 'summary']) {
                    ...
                    const position = await page.evaluate(() => {
                        ...
                        return { top: root.scrollTop, documentTop: window.scrollY, left: root.scrollLeft, width: root.clientWidth, scrollWidth: root.scrollWidth };
                    });
                    assert.equal(position.top, 0, `${href}: column viewport must not scroll vertically`);
                    assert.equal(position.documentTop, 0, `${href}: document must not scroll vertically`);
                    assert.equal(Math.abs(position.left) % position.width, 0, `${href}: complete page alignment`);
                    if (href === '#back' || href === '#') assert.equal(position.left, 0);
                    else assert.ok(direction === 'rtl' ? position.left < 0 : position.left > 0, JSON.stringify(position));
                }
                await page.close();
            });
        }
